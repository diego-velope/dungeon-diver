...
impl Player {
    ...
    fn try_move(&mut self, dir: Direction, level: &Level) {
        ...
        if level.is_valid(new_x, new_y) {
            self.facing = dir;
            self.move_start_x = self.grid_x;
            self.move_start_y = self.grid_y;
            self.grid_x = new_x;
            self.grid_y = new_y;
            self.is_moving = true;
            self.move_progress = 0.0;
    ...
    pub fn respawn(&mut self, spawn_x: i32, spawn_y: i32) {
        ...
        self.is_moving = false;
        self.move_progress = 0.0;
        self.move_start_x = spawn_x;
        self.move_start_y = spawn_y;
        self.invincible_time = PLAYER_INVINCIBLE_TIME;
    ...
    pub fn draw(&self, camera_x: f32, camera_y: f32) {
        ...
        if let (Some(ref idle), Some(ref run)) = (&self.idle_sprite, &self.run_sprite) {
            ...
            // Reuse LEFT-facing row and mirror only when actually facing RIGHT.
            // In this spritesheet orientation, flipping for LEFT keeps visuals correct.
